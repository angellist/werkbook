...
func main() {
	...
	output.SetValue("B1", "Total Absolute Amount")
	output.SetValue("C1", "Transaction Count")
	...
	for i, typ := range types {
		...
		// SUMIF to total absolute amounts by type.
		sumFormula := fmt.Sprintf(
			"SUMIF(Ledger!B2:B%d,A%d,Ledger!E2:E%d)",
			lastRow, row, lastRow,
		)
		output.SetFormula(fmt.Sprintf("B%d", row), sumFormula)

		// COUNTIF to count transactions by type.
		countFormula := fmt.Sprintf(
			"COUNTIF(Ledger!B2:B%d,A%d)",
			lastRow, row,
		)
		output.SetFormula(fmt.Sprintf("C%d", row), countFormula)
	}
